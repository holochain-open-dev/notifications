...
2. Add the `hc_zome_notifications_coordinator` and `hc_zome_notifications_integrity` zomes as dependencies with:

```bash
cargo add -p notifications hc_zome_notifications_coordinator
cargo add -p notifications_integrity hc_zome_notifications_integrity
```
